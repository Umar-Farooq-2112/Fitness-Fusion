...
router.post('/diet/keywords/:w_id', (req, res) => {
...
router.post('/workout/keywords/:w_id', (req, res) => {
    const w_id = req.params.w_id; // Get workout ID from URL
    const keywords = req.body.ids; // Extract keywords from JSON body
...
router.post('/workout/match/:user_id', (req, res) => {
    const user_id = req.params.user_id;
    const { ids } = req.body; 
    console.log(ids)
...
router.post('/diet/match/:user_id', (req, res) => {
    const user_id = req.params.user_id;
    const { ids } = req.body; 
    console.log(ids)
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
